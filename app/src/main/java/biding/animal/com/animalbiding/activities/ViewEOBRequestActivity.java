...
public class ViewEOBRequestActivity extends AppCompatActivity implements VolleyResponseInterface, View.OnClickListener {

    private static final int GET_REQUEST_TAG = 1;
    private ImageView mBack;
    private RecyclerView mRequestListView;
    private ProgressBar mProgressBar;
    private ViewRequestAdapter eobRequestAdapter;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_eobdetail);
    ...
    private void getEOBRequest() {
        ...
        volleyClient.hitWithOutTokenService(Constants.GET_ALL_ASSOCIATE_EOB_REQUEST + "?userid=" + SharedPrefernceManger.getUserId(), null, GET_REQUEST_TAG);
    }

    //Method to set recycler adapter...
    private void setEOBRequestAdapter(List<RequestModel> featureList) {
        LinearLayoutManager layoutManager = new LinearLayoutManager(this, LinearLayoutManager.VERTICAL, false);
        mRequestListView.setLayoutManager(layoutManager);
        eobRequestAdapter = new ViewRequestAdapter(ViewEOBRequestActivity.this, featureList, false);
        mRequestListView.setAdapter(eobRequestAdapter);
    }
                    ...
                    if (object != null) {
                        if (object.has("lstRequest") && object.getJSONArray("lstRequest") != null) {
                            ...
                            if (detailArray.length() > 0) {
                                ...
                                setEOBRequestAdapter(postModelList);
                            } else {
                                Toast.makeText(ViewEOBRequestActivity.this, getString(R.string.no_detail_found), Toast.LENGTH_LONG).show();
                            }
                        }
                    }
    ...
    public void onError(VolleyError error, int tag) {
        switch (tag) {
            case GET_REQUEST_TAG:
                Toast.makeText(ViewEOBRequestActivity.this, R.string.some_problem_occurred, Toast.LENGTH_LONG).show();
                break;
        }
    }
